...
from pybridge.games.bridge import pbn
...
class Bridge(object):
    ...
    def __init__(self, **options):
        ...
        self.pbnLibrary=pbn.PbnLibrary()
        if not len(self.pbnLibrary.games):
            self.pbnLibrary=None 
...
# Implementation of ICardGame.
    ...
    def start(self, board=None):
        ...
        if self.pbnLibrary:
            current_deal=self.pbnLibrary.current
            pbn_info=self.pbnLibrary.games[current_deal].MTS
            self.pbnLibrary.current+=1
            dealstr=pbn_info['Deal']
            dealer=Direction[pbn_info['Dealer']]
            vulnerable=pbn_info['Vulnerable']
            if vulnerable=='NS':
                vulnerable='NorthSouth'
            elif vulnerable=='EW':
                vulnerable='EastWest'
            vuln=Vulnerable[vulnerable]
            #
            deal=Deal.fromString(dealstr)
        else:
            deal=None
            dealer=None
            vuln=None
        ...
        if board:  # Use specified board.
            self.board = board
            print "board: use specified board"
        elif self.boardQueue:  # Use pre-specified board.
            self.board = self.boardQueue.pop(0)
            print "board: use pre-specified board"
        elif self.board:  # Advance to next round.
            print "board: use next board"
            self.board = self.board.next(deal=deal,dealer=dealer,vuln=vuln)
        else:  # Create an initial board.
            print "board: create board"
            self.board = Board.first(deal=deal,dealer=dealer,vuln=vuln)
        if self.options.get('RubberScoring'):
            ...
            elif self.pbnLibrary:
                pass # taken care of in setting the deal
            else:
